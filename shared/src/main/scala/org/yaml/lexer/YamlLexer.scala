...
final class YamlLexer private (
  ...
  private def implicitOrExplicitDocument(): Boolean =
    zeroOrMore(directive()) && (explicitDocument() && optional(documentSuffix()) || bareDocument())
  ...
  /** Outside scalar content, YAML allows any line break to be used to terminate lines.<p> [30] b-non-content ::=
    * b-break
    */
  @failfast def breakNonContent(): Boolean = lineBreak(LineBreak)

  /** URI characters for tags, as specified in RFC2396, with the addition of the “[” and “]” for presenting IPv6
    * addresses as proposed in RFC2732.<p> [39] ns-uri-char ::= “%” ns-hex-digit ns-hex-digit | ns-word-char \| “#” |
    * “;” | “/” | “?” | “:” | “@” | “&” | “=” | “+” | “$” \| “,” | “_” | “.” | “!” | “~” | “*” | “'” | “(” | “)” | “[” |
    * “]”
    */
  ...
  /** A Tag char cannot contain the “!” character because is used to indicate the end of a named tag handle. In
    * addition, the “[”, “]”, “{”, “}” and “,” characters are excluded because they would cause ambiguity with flow
    * collection structures.<p>
    *
  ...
  /** Process an indentation exactly as the current one<p> [63] s-indent(n) ::= s-space × n
    */
  ...
  /** <blockquote><pre> [67] s-line-prefix(n,c) ::= c = block-out ⇒ s-block-line-prefix(n) c = block-in ⇒
    * s-block-line-prefix(n) c = flow-out ⇒ s-flow-line-prefix(n) c = flow-in ⇒ s-flow-line-prefix(n) [68]
    * s-block-line-prefix(n) ::= s-indent(n) [69] s-flow-line-prefix(n) ::= s-indent(n) s-separate-in-line? [70]
    * l-empty(n,c) ::= ( s-line-prefix(n,c) | s-indent(&lt;n) ) b-as-line-feed
    *
    * </blockquote></pre>
    */
  ...
  /** If a line break is followed by an empty line, it is trimmed. The first line break is discarded and the rest are
    * retained as content.<p> [71] b-l-trimmed(n,c) ::= [[breakNonContent b-non-content]] [[emptyLine l-empty(n,c)]]+
    */
  private def trimmed(n: Int, ctx: YamlContext): Boolean = matches(breakNonContent() && oneOrMore(emptyLine(n, ctx)))

  /** Convert a lineBreak to an space (fold it)<p> [72] b-as-space ::= b-break
    */
  def breakAsSpace(): Boolean = lineBreak(LineFold)

  /** A folded non-empty line may end with either of the above line breaks.<p> [73] b-l-folded(n,c) ::=
    * [[trimmed b-l-trimmed(n,c)]] | [[breakAsSpace b-as-space]]
    */
  private def folded(n: Int, ctx: YamlContext): Boolean =
    isBBreak(currentChar) && (matches(trimmed(n, ctx)) || breakAsSpace())

  /** The combined effect of the flow line folding rules is that each “paragraph” is interpreted as a line, empty lines
    * are interpreted as line feeds, and text can be freely more-indented without affecting the content information<p>
    *
    * [74] s-flow-folded(n) ::= [[separateInLine s-separate-in-line]]? [[folded b-l-folded(n,flow-in)]]
  ...
  /** [77] s-b-comment ::= ( s-separate-in-line c-nb-comment-text? )? b-comment* Actually refactored to: b-comment |
    * l-comment
    */
  ...
  /** <blockquote><pre>
    ...
    *
    * Actually refactored to: [[separateInLine s-separate-in-line]] ([[breakComment b-non-content]] | [[commentText]])
    * </blockquote></pre>
    */
  ...
  /** [80] s-separate(n,c) ::= c = block-out ⇒ s-separate-lines(n) c = block-in ⇒ s-separate-lines(n) c = flow-out ⇒
    * s-separate-lines(n) c = flow-in ⇒ s-separate-lines(n) c = block-key ⇒ s-separate-in-line c = flow-key ⇒
    * s-separate-in-line [81] s-separate-lines(n) ::= ( s-l-comments s-flow-line-prefix(n) ) | s-separate-in-line
    */
  ...
  /** Directives are instructions to the YAML processor. [82] l-directive ::= “%” ( [[yamlDirective ns-yaml-directive]]
    * \| [[tagDirective ns-tag-directive]] \| [[reservedDirective ns-reserved-directive]] ) s-l-comments
    */
  ...
  /** Each directive is specified on a separate non-indented line starting with the “%” indicator, followed by the
    * directive name and a list of parameters. <blockquote><pre>
    *
    * [83] ns-reserved-directive ::= ns-directive-name ( s-separate-in-line ns-directive-parameter )* [84]
    * ns-directive-name ::= ns-char+ [85] ns-directive-parameter ::= ns-char+
    *
    * </blockquote></pre>
    */
  ...
  /** [86] ns-yaml-directive ::= “Y” “A” “M” “L” s-separate-in-line ns-yaml-version [87] ns-yaml-version ::=
  ...
  /** The “TAG” directive establishes a tag shorthand notation for specifying node tags. Each “TAG” directive associates
    * a handle with a prefix. This allows for compact and readable tag notation.<p> [88] ns-tag-directive ::= “T” “A”
    * “G” [[separateInLine s-separate-in-line]] [[tagHandle c-tag-handle]] [[separateInLine s-separate-in-line]]
  ...
  /** The tag handle exactly matches the prefix of the affected tag shorthand. There are three tag handle variants:<p>
    *
    * [89] c-tag-handle ::= c-named-tag-handle | c-secondary-tag-handle | c-primary-tag-handle [91]
    * c-secondary-tag-handle ::= “!” “!” [92] c-named-tag-handle ::= “!” ns-word-char+ “!”
    */
  ...
  /** There are two tag prefix variants:<p> Local Tag Prefix: If the prefix begins with a “!” character, shorthands
    * using the handle are expanded to a local tag.<p> Global Tag Prefix: If the prefix begins with a character other
    * than “!”, it must to be a valid URI prefix<p> <blockquote><pre>
    *
    * [93] ns-tag-prefix ::= c-ns-local-tag-prefix | ns-global-tag-prefix [94] c-ns-local-tag-prefix ::= “!”
    * ns-uri-char* [95] ns-global-tag-prefix ::= ns-tag-char ns-uri-char*
    *
    * </blockquote></pre>
    */
  ...
  /** Each node may have two optional properties, anchor and tag, in addition to its content. Node properties may be
    * specified in any order before the node’s content. Either or both may be omitted.<p> <blockquote><pre>
    *
    * [96] c-ns-properties(n,c) ::= ( [[tagProperty c-ns-tag-property]] ( [[separate s-separate(n,c)]]
    * [[anchorProperty c-ns-anchor-property]] )? ) \| ( [[anchorProperty c-ns-anchor-property]] (
    * [[separate s-separate(n,c)]] [[tagProperty c-ns-tag-property]] )? ) </blockquote></pre>
    */
  ...
  /** The tag property identifies the type of the native data structure presented by the node. A tag is denoted by the
    * “!” indicator. <blockquote><pre>
    *
    * [97] c-ns-tag-property ::= c-verbatim-tag | c-ns-shorthand-tag | c-non-specific-tag [98] c-verbatim-tag ::= “!”
    * “<” [[uriChar ns-uri-char]]+ “>” [99] c-ns-shorthand-tag ::= [[tagHandle c-tag-handle]] [[tagChar ns-tag-char]]+
  ...
  /** An anchor is denoted by the “&” indicator. It marks a node for future reference. An alias node can then be used to
    * indicate additional inclusions of the anchored node. An anchored node need not be referenced by any alias nodes;
    * in particular, it is valid for all nodes to be anchored.<p>
    *
  ...
  /** An anchor name<p> [103] ns-anchor-name ::= [[isNsAnchorChar ns-anchor-char]]+
    */
  ...
  /** An alias node is denoted by the “*” indicator. The alias refers to the most recent preceding node having the same
    * anchor.<p>
    *
  ...
  @failfast private def plainFirst(ctx: YamlContext): Boolean = {
    val chr = currentChar
    val result =
      isNsChar(chr) && !isIndicator(chr) || (chr == '-' || chr == ':' || chr == '?') && isPlainSafe(lookAhead(1), ctx)
    if (result) consume()
    result
  }

  /** [127] ns-plain-safe(c) ::= c = flow-out ⇒ ns-plain-safe-out c = flow-in ⇒ ns-plain-safe-in c = block-key ⇒
    * ns-plain-safe-out c = flow-key ⇒ ns-plain-safe-in [128] ns-plain-safe-out ::= ns-char [129] ns-plain-safe-in ::=
  ...
  /** [130] ns-plain-char(c) ::= ( ns-plain-safe(c) - “:” - “#” ) \| ( /* An ns-char preceding */ “#” ) \| ( “:” /*
  ...
  /** [131] ns-plain(n,c) ::= c = flow-out ⇒ ns-plain-multi-line(n,c) c = flow-in ⇒ ns-plain-multi-line(n,c) c =
    * block-key ⇒ ns-plain-one-line(c) c = flow-key ⇒ ns-plain-one-line(c)
    */
  ...
  /** [134] s-ns-plain-next-line(n,c) ::= s-flow-folded(n) ns-plain-char(c) nb-ns-plain-in-line(c) [135]
    * ns-plain-multi-line(n,c) ::= ns-plain-one-line(c) s-ns-plain-next-line(n,c)*
    */
  ...
  /** Flow sequence content is denoted by surrounding “[” and “]” characters.<p>
    *
    * <blockquote><pre> [136] in-flow(c) ::= c = flow-out ⇒ flow-in c = flow-in ⇒ flow-in c = block-key ⇒ flow-key c =
    * flow-key ⇒ flow-key
    */
  ...
  /** Flow sequence content is denoted by surrounding “[” and “]” characters.<p>
    *
    * [137] c-flow-sequence(n,c) ::= “[” [[separate s-separate(n,c)]]? [\[flowSequenceEntries
  ...
  /** Sequence entries are separated by a “,” character.<p>
    *
    * [138] ns-s-flow-seq-entries(n,c) ::= [[flowSequenceEntry ns-flow-seq-entry(n,c)]] [[separate s-separate(n,c)]]? (
    * “,” [[separate s-separate(n,c)]]? [[flowSequenceEntries ns-s-flow-seq-entries(n,c)]]? )?
    */
  ...
  /** Flow mappings are denoted by surrounding “{” and “}” characters.<p>
    *
    * [140] c-flow-mapping(n,c) ::= “{” [[separate s-separate(n,c)]]?
  ...
  /** Mapping entries are separated by a “,” character.<p>
    *
    * [141] ns-s-flow-map-entries(n,c) ::= ns-flow-map-entry(n,c) s-separate(n,c)? ( “,” s-separate(n,c)?
  ...
  /** A Flow Map Entry<p> If the optional “?” mapping key indicator is specified, the rest of the entry may be
    * completely empty.<p>
    *
    * <blockquote><pre> [142] ns-flow-map-entry(n,c) ::= ( “?” [[separate s-separate(n,c)]]
  ...
  /** <blockquote><pre> [144] ns-flow-map-implicit-entry(n,c) ::= ns-flow-map-yaml-key-entry(n,c) \|
    * [[flowMapEmptyKeyEntry c-ns-flow-map-empty-key-entry(n,c)]] \|
    * [[flowMapJsonKeyEntry c-ns-flow-map-json-key-entry(n,c)]]
    *
    * [145] ns-flow-map-yaml-key-entry(n,c) ::= [[flowYamlNode ns-flow-yaml-node(n,c)]] ( ([[separate s-separate(n,c)]]?
    * [[flowMapSeparateValue c-ns-flow-map-separate-value(n,c)]]) \| [[emptyNode e-node]] )
    *
    * </blockquote></pre> // todo complete
    */
  ...
  /** [147] c-ns-flow-map-separate-value(n,c) ::= “:” (Not followed by an [[isPlainSafe ns-plain-safe(c)]] ) ( (
    * [[separate s-separate(n,c)]] [[flowNode ns-flow-node(n,c)]]) \| [[emptyNode() e-node]] )
    */
  ...
  /** [148] c-ns-flow-map-json-key-entry(n,c) ::= [[flowJsonNode c-flow-json-node(n,c)]] ( (
    * [[separate s-separate(n,c)]]? [[flowMapAdjacentValue c-ns-flow-map-adjacent-value(n,c)]] ) \| [[emptyNode e-node]]
    * )
    */
  ...
  /** [149] c-ns-flow-map-adjacent-value(n,c) ::= “:” ( ( [[separate s-separate(n,c)]]? [[flowNode ns-flow-node(n,c)]] )
    * \| [[emptyNode e-node]] )
    */
  ...
  /** If the “?” indicator is explicitly specified, parsing is unambiguous, and the syntax is identical to the general
    * case. <p>
    *
    * [150] ns-flow-pair(n,c) ::= ( “?” [[separate s-separate(n,c)]]
    * [[flowMapExplicitEntry ns-flow-map-explicit-entry(n,c)]] ) \| [[flowPairEntry ns-flow-pair-entry(n,c)]]
    */
  private def flowPair(n: Int, c: YamlContext): Boolean =
    indicator('?') && separate(n, c) && flowMapExplicitEntry(n, c) || flowPairEntry(n, c)

  /** <blockquote><pre> [151] ns-flow-pair-entry(n,c) ::= ns-flow-pair-yaml-key-entry(n,c) \|
    * [[flowMapEmptyKeyEntry c-ns-flow-map-empty-key-entry(n,c)]] \| c-ns-flow-pair-json-key-entry(n,c)
    *
  ...
  /** JSON-like flow styles all have explicit start and end indicators.<p> <blockquote><pre> [157]
    * c-flow-json-content(n,c) ::= [[flowSequence c-flow-sequence(n,c)]] \| [[flowMapping c-flow-mapping(n,c)]] \|
    * [[quotedScalar c-single-quoted(n,c)]] \| [[quotedScalar c-double-quoted(n,c)]] </blockquote></pre>
    */
  ...
  /** A complete flow node also has optional node properties, except for alias nodes which refer to the anchored node
    * properties.<p>
    *
    * [161] ns-flow-node(n,c) ::= [[aliasNode c-ns-alias-node]] \| [[flowContent ns-flow-content(n,c)]] \| (
    * [[nodeProperties c-ns-properties(n,c)]] ( ( [[separate s-separate(n,c)]] [[flowContent ns-flow-content(n,c) )]])
    * \| [[emptyScalar e-scalar]] )
    */
  ...
  /** Returns the new defined indentation or MAX_VALUE if autodetect <blockquote><pre> [162] c-b-block-header(m,t) ::= (
    * ( c-indentation-indicator(m) c-chomping-indicator(t) ) \| ( c-chomping-indicator(t) c-indentation-indicator(m) ) )
    * s-b-comment
    *
    * [163] c-indentation-indicator(m) ::= ns-dec-digit ⇒ m = ns-dec-digit - #x30 \| Empty
    *
    * [164] c-chomping-indicator(t) ::= “-” ⇒ t = strip \| “+” ⇒ t = keep \| “ ” ⇒ t = clip
    *
    * </blockquote></pre>
    */
  ...
  /** Process the last block of an Scalar based on the chomp indicator
    ...
    * <blockquote><pre>
    *
    * [165] b-chomped-last(t) ::= t = strip(-) ⇒ b-non-content | EOF t = clip ⇒ b-as-line-feed | EOF t = keep (+) ⇒
    * b-as-line-feed | EOF </blockquote></pre>
    */
  ...
  /** <blockquote><pre>
    *
    * [166] l-chomped-empty(n,t) ::= t = strip(-) ⇒ l-strip-empty(n) t = clip ⇒ l-strip-empty(n) t = keep (+) ⇒
    * l-keep-empty(n)
    *
    * [167] l-strip-empty(n) ::= ([[indentLessOrEqual s-indent(≤n)]] [[breakNonContent b-non-content]])*
    * [[trailComments l-trail-comments(n)]]?
    *
    * [168] l-keep-empty(n) ::= [[emptyLine l-empty(n,block-in)]]* [[trailComments l-trail-comments(n)]]?
    * </blockquote></pre>
    */
  ...
  /** Explicit comment lines may follow the trailing empty lines. To prevent ambiguity, the first such comment line must
    * be less indented than the block scalar content. Additional comment lines, if any, are not so restricted. This is
    * the only case where the indentation of comment lines is constrained.<p>
    *
    * [169] l-trail-comments(n) ::= [[indentLess s-indent(&lt;n)]] [[commentText c-nb-comment-text b-comment]]
  ...
  /** <blockquote><pre> [173] l-literal-content(n,t) ::= ( [[literalContentLine l-nb-literal-text(n)]]
    * b-nb-literal-next(n) * [[chompedLast b-chomped-last(t)]] )? [[chompedEmpty l-chomped-empty(n,t)]]
    *
  ...
  /** The folded style is denoted by the “>” indicator. It is similar to the literal style; however, folded scalars are
    * subject to line folding.<p> [174] c-l+folded(n) ::= “>” [[blockHeader c-b-block-header(m,t)]] [\[foldedContent
  ...
  /** [176] l-nb-folded-lines(n) ::= [[foldedText s-nb-folded-text(n)]] ([[folded b-l-folded(n,block-in)]]
    * [[foldedText s-nb-folded-text(n)]] )*
    */
  ...
  /** Lines starting with white space characters (more-indented lines) are not folded.<p> [177] s-nb-spaced-text(n) ::=
    * [[indent s-indent(n)]] [[isWhite s-white]] [[isNBChar nb-char]]*
    */
  ...
  /** <blockquote><pre> [178] b-l-spaced(n) ::= [[breakAsLineFeed b-as-line-feed]] [[emptyLine l-empty(n,block-in)]]*
    * [179] l-nb-spaced-lines(n) ::= [[spacedText s-nb-spaced-text(n)]] ( b-l-spaced(n)
    * [[spacedText s-nb-spaced-text(n)]] )* </blockquote></pre>
    */
  ...
  /** Line breaks and empty lines separating folded and more-indented lines are also not folded.<p> [180]
    * l-nb-same-lines(n) ::= [[emptyLine l-empty(n,block-in)]]* ( [[foldedLines l-nb-folded-lines(n)]] \|
  ...
  /** The final line break, and trailing empty lines if any, are subject to chomping and are never folded.<p>
    * <blockquote><pre> [181] l-nb-diff-lines(n) ::= [[sameLines l-nb-same-lines(n)]] (
  ...
  /** A block sequence is simply a series of nodes, each denoted by a leading “-” indicator. The “-” indicator must be
    * separated from the node by white space. This allows “-” to be used as the first character in a plain scalar if
    * followed by a non-space character (e.g. “-1”).<p>
    *
    * <blockquote><pre> [183] l+block-sequence(n) ::= ( [[indent s-indent(n+m)]]
    * [[blockSeqEntry c-l-block-seq-entry(n+m)]] )+ (For some fixed auto-detected m > 0) </blockquote></pre>
    */
  ...
  private def errorUntilSeq(n: Int, m: Int) =
    oneOrMoreErrorSeqEntries(n, m, () => (!looksLikeSeq(n, m) && input.countSpaces() > n + m)) && looksLikeSeq(
        n,
        m
    ) && emitError()

  private def errorsUntilBack(n: Int, m: Int) =
    oneOrMoreErrorSeqEntries(n, m, () => seqIndentedFromFather(n)) && emitError() || !nonEof

  private def seqIndentedFromFather(n: Int): Boolean = input.countSpaces() > n + 1

  private def oneOrMoreErrorSeqEntries(n: Int, m: Int, whileFN: () => Boolean): Boolean = oneOrMore(
      whileFN() && checkSeqIndent(n, m) && seqEntryError(n)
  )
  ...
  /** [184] c-l-block-seq-entry(n) ::= “-” (Not followed by an ns-char) [[blockIndented s-l+block-indented(n,block-in)]]
    * </blockquote></pre>
    */
  private def blockSeqEntry(n: Int) = indicator('-') && blockIndented(n, BlockIn)

  /** The entry node may be either completely empty, be a nested block node, or use a compact in-line notation.<p> The
    * compact notation may be used when the entry is itself a nested block collection.<p>
    *
    * <blockquote><pre> [185] s-l+block-indented(n,c) ::= ([[indent s-indent(m)]] (
    * [[compactSequence ns-l-compact-sequence(n+1+m)]] \| [[compactMapping ns-l-compact-mapping(n+1+m)]] ) ) \|
    * [[blockNode s-l+block-node(n,c)]] \| ( [[emptyNode e-node]] [[multilineComment s-l-comments]] )
    *
    * </blockquote></pre>
    */
  ...
  /** [186] ns-l-compact-sequence(n) ::= [[blockSeqEntry c-l-block-seq-entry(n)]] ( [[indent s-indent(n)]]
    * [[blockSeqEntry c-l-block-seq-entry(n)]] )*
    */
  ...
  /** A Block mapping is a series of entries, each presenting a key: value pair.<p> [187] l+block-mapping(n) ::= (
    * [[indent s-indent(n+m)]] [[blockMapEntry ns-l-block-map-entry(n+m)]] )+ <p> For some fixed auto-detected m greater
    * than 0
    */
  ...
  private def someEntryIndicator(): Boolean =
    explicitEntry || lineContainsMapIndicator() || isFlowOrPlainIndicator(currentChar)

  private def explicitEntry: Boolean = currentChar == '?'

  /** [188] ns-l-block-map-entry(n) ::= [[mapExplicitEntry c-l-block-map-explicit-entry(n)]] \|
  ...
  /** Explicit map entries are denoted by the “?” mapping key indicator<p> <blockquote><pre> [189]
    * c-l-block-map-explicit-entry(n) ::= c-l-block-map-explicit-key(n) ( l-block-map-explicit-value(n) |
    * [[emptyNode e-node]] )
    *
  ...
  /** [191] l-block-map-explicit-value(n) ::= [[indent s-indent(n)]] “:” [\[blockIndented
  ...
  /** Detect a Map implicit Entry
    ...
    * <blockquote><pre>
    *
    * [192] ns-l-block-map-implicit-entry(n) ::= ( ns-s-block-map-implicit-key | [[emptyNode e-node]] )
  ...
  /** [195] ns-l-compact-mapping(n) ::= [[blockMapEntry ns-l-block-map-entry(n)]] ( [[indent s-indent(n)]]
    * [[blockMapEntry ns-l-block-map-entry(n)]] )*
    */
  def compactMapping(n: Int): Boolean =
    emit(BeginMapping) && blockMapEntry(n) && zeroOrMore(indent(n) && blockMapEntry(n)) && emit(EndMapping)

  /** [196] s-l+block-node(n,c) ::= s-l+block-in-block(n,c) | s-l+flow-in-block(n) [197] s-l+flow-in-block(n) ::=
  ...
  /** A Block Scalar Node<p> [199] s-l+block-scalar(n,c) ::= [[separate s-separate(n+1,c)]] ([[nodeProperties
    * c-ns-properties(n+1,c)]] [[separate s-separate(n+1,c)]])? ( [[literalScalar c-l+literal(n)]] |
  ...
  /** A nested Block Collection<p> <blockquote><pre> [200] s-l+block-collection(n,c) ::= (
    * [[separate s-separate(n+1,c)]] c-ns-properties(n+1,c) )? [[multilineComment s-l-comments]] (
    * [[blockSequence l+block-sequence(seq-spaces(n,c))]] \| [[blockMapping]] l+block-mapping(n) )
    *
  ...
  /** A document may be preceded by a prefix specifying the character encoding, and optional comment lines. Note that
    * all documents in a stream must use the same character encoding.<p>
    *
  ...
  /** * The Document End marker. Specifying false for the parameter will only test without emitting<p> [204]
    * c-document-end ::= “.” “.” “.”
    */
  ...
  /** An explicit document begins with an explicit directives end marker line but no directives. Since the existence of
    * the document is indicated by this marker, the document itself may be completely empty.<p>
    *
    * [208] l-explicit-document ::= [[isDirectivesEnd c-directives-end]] ( [[bareDocument l-bare-document]] \| (
    * [[emptyNode e-node]] [[multilineComment s-l-comments]] ) )
    */
  ...
  /** A YAML stream consists of zero or more documents. Subsequent documents require some sort of separation marker
    * line. If a document is not terminated by a document end marker line, then the following document must begin with a
    * directives end marker line. The stream format is intentionally “sloppy” to better support common use cases, such
    * as stream concatenation.
    *
    * <blockquote><pre>
    *
    * [211] l-yaml-stream ::= [[documentPrefix l-document-prefix]]* [[anyDocument l-any-document]]? ( l-document-suffix+
    * l-document-prefix* l-any-document? \| l-document-prefix* l-explicit-document? )* </blockquote></pre>
    */
...
object YamlLexer {
  ...
  def apply(cs: CharSequence, positionOffset: Position): YamlLexer =
    new YamlLexer(CharSequenceLexerInput(cs), positionOffset, None)
  ...
  def apply(cs: CharSequence, maxDepth: Option[Int]): YamlLexer =
    new YamlLexer(CharSequenceLexerInput(cs), maxDepth = maxDepth)
  def apply(cs: CharSequence, sourceName: String, maxDepth: Option[Int]): YamlLexer =
    new YamlLexer(CharSequenceLexerInput(cs, sourceName = sourceName), maxDepth = maxDepth)
  def apply(cs: CharSequence, positionOffset: Position, maxDepth: Option[Int]): YamlLexer =
    new YamlLexer(CharSequenceLexerInput(cs), positionOffset, maxDepth)
  def apply(cs: CharSequence, sourceName: String, positionOffset: Position, maxDepth: Option[Int]): YamlLexer =
    new YamlLexer(CharSequenceLexerInput(cs, sourceName = sourceName), positionOffset, maxDepth)
